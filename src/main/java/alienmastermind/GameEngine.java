...
// for (int i = 0; i < 4; i++) // go through each peg in sequence
// {
// if (((Peg) currentPegSeq.get(i)).getColor() ==
// (target.getSequence(i).getColor())) {
// numBlueRockets++;
// target.setPegCount(i, Goal.COUNTEDBLUE);
// }
// }
// for (int i = 0; i < 4; i++) // go through each peg again to test for white
// rockets
// {
// goThruTarget: for (int j = 0; j < 4; j++) // go through each peg in target
// until there is a match or target
// // sequence has been exhaustively searched
// {
// if ((((Peg) currentPegSeq.get(i)).getColor() ==
// (target.getSequence(j).getColor()))
// && (target.getPegCount(i) != Goal.COUNTEDBLUE) && (target.getPegCount(j) ==
// Goal.NOTCOUNTED)) {
// numWhiteRockets++;
// target.setPegCount(j, Goal.COUNTEDWHITE);
// break goThruTarget;
